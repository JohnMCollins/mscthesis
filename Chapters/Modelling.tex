...
to generate synthetic spectra, with some modifications. Specifically, \forcomment{a 3D model of the star, but with a 2D
...
other stars, the same \forcomment{centre-to-limb variation as for the solar plage was used relative to the photospheric
...
narrow width to represent the central self-absorption as illustrated in Fig. \ref{fig:integregions}.  \forcomment{The
  profile was selected with width resembling the {\ha} peak and with equivalent width so as to resemble an mean peak in
  the {\harps} data. The intensity could be fine-adjusted in the lookup-table parameters for the DoTS program so as to
  yield a variation in equivalent width comparable to that of the {\harps} observations ignoring those affected by
  flares with the selected plage distribution in use.}
...
since the local intensity profile for {\ha} \forcomment{possesses a normalised peak intensity of the set number of
  times} the continuum. For stars with rotational velocity much greater than the instrument resolution, line asymmetries
are likely to be much more reliable.
...
large spot on one face to randomly-placed spots of random sizes. \examrevision{In all cases a 100\% filling factor of
...
\forcomment{with the intensity parameters in the flux profile and the model adjusted to yield a variation in equivalent
