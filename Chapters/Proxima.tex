...
\examrevision{Fig. \ref{fig:harps1} illustrates the regions used to calculate various measurements discussed in this
  \paperorthesis. To calculate equivalent widths of the {\ha} peak, points in the range delineated by the dark read
...
\examrevision{Also calculated is what is herein called the ``Peak Ratio'', defined as the ratio of the mean values of
  ...
  mean values of the selected points, which does not depend greatly on the actual number in the selected range. These
  regions were chosen to optimise variability in the line profiles. In the observed data the ``red'' {\horn} tends to be
  higher but narrower than the ``blue'' \horn.  For similar reasons, there is no need to interpolate points to the
  boundaries of the region for the peak ratio measurement.}
...
widths. \examrevision{In Table \ref{table:divcompar} are recalculated the figures for the median residual equivalent widths and peak
ratios for {\harps} which were previously displayed in Table \ref{table:ewtabfirst}}. The same two spectra which are
