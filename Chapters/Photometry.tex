...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[scale=0.40]{Figures/asasnobin.png} \\
\end{center}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[scale=0.40]{Figures/asaswf3.png} \\
\end{center}
\caption{\examrevisionb{This figure displays the window function of the observation times of the {\asas} data from
    {\prox} as displayed in Fig. \ref{fig:asaspgram1}, calculated using \textit{Period04}. There is a strong peak at 0.0027
    c/day, which is a period of approximately 370 days.}}
\protect\label{fig:asaswf1}
\end{figure}

\begin{figure}[!htbp]
\begin{center}
\includegraphics[scale=0.40]{Figures/asaswf2.png} \\
\end{center}
\caption{\examrevisionb{This figure redisplays together in the upper panel the main peaks in the periodogram of the {\asas} data from
    {\prox} as displayed in Fig. \ref{fig:asaspgram1}, calculated using Period04 and on a cycle/day X axis, together
    with the window function on the same X axis scale in the lower panel.}}
\protect\label{fig:asaswf2}
\end{figure}
...
\examrevision{The 163.3-day period is close to double the 82.6-day period reported in the papers previously discussed
  ...
  not likely to be a sub-harmonic of this.}

\examrevisionb{In Fig. \ref{fig:asaswf1} is displayed the window function of the observation times of this data,
  calculated using \textit{Period04}\footnote{\examrevisionb{Obtainable from https://www.univie.ac.at/tops/Period04/}},
  showing a clear peak of 0.0027 c/day, equivalent to a period of 370 days\footnote{\examrevisionb{The periods given by
      \textit{Period04} are up to 1.3\% different from the Lomb-Scargle based routines.}}. It was also found useful to
  re-plot the periodogram and the window function on the same scale, as shown in Fig. \ref{fig:asaswf2}, calculated
  using \textit{Period04}, as the window function shows the same clear peak of 0.0027. Subtracting this frequency from
  the strongest period of 0.0121 c/day in the periodogram (corresponding to the 82.6-day period) gives 0.0094 c/day,
  which is the next strongest peak in the periodogram corresponding to the 106.3-day period in
  Fig. \ref{fig:asaspgram1}. Taking into account the methods described in \citet{dawson10}, it is possible to express
  confidence that 82.6 days is a genuine period and the 106.3 day period an alias in consequence of the strong 0.0027
  c/day peak in the window function.}
...
\examrevisionb{Construction of a phase-folded light curve from the 18-minute binned data yields Fig. \ref{fig:asaspflc}.}
...
\end{center}
\caption{\examrevisionb{In the upper panel is displayed a periodogram derived from the same data as Fig. \ref{fig:hstb4min}
    calculated with \textit{Period04} and with an X-axis of cycles/day. In the lower panel is displayed the
...
\examrevision{gave almost identical periodograms} (other than the scaling of the power). \examrevision{A phase-folded
  ...
  observation times gave the result shown in Fig. \ref{fig:hstwf}.}. \examrevisionb{The window function peak at 0.0007
  cycles/day would appear to account for the peak of around 78 days in Fig. \ref{fig:hstb4min} which corresponds to
  0.0129 cycles/day in the in upper panel of Fig. \ref{fig:hstwf}.}
...
\examrevisionb{This corresponds to the findings from the study of the window function in Fig. \ref{fig:asaswf2}. Similarly the strong
77.8-day period seen in the {\hst} data but not in the {\asas} data appears to be related to the 0.0007 cycles/day peak
in the window function illustrated in Fig. \ref{fig:hstwf}, so again}
\examrevisionb{it is concluded to be an alias thrown up by the observation times.}
