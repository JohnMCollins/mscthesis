...
\examrevisiona{The window functions displayed in Figs. \ref{fig:asaswf1}, \ref{fig:hstwf}, \ref{fig:harpswfos} and
  \ref{fig:harpswffs} were derived using the \textit{Numerical Recipes} routine. It was not possible to use any of the
  Python routines described in Section \ref{section:pythonls} to calculate the window functions by supplying a
  constant value of intensity, all either generated overflows or reported zero values.  The calculated window functions
  were confirmed by using   \textit{Period04}\footnote{\examrevisiona{https://www.univie.ac.at/tops/Period04/}}. \textit{Period04} was not used
  throughout as it is a GUI-based program and many of the calculations in this {\paperorthesis} required a repetitive
  ``batch mode'' mode of operation.}
